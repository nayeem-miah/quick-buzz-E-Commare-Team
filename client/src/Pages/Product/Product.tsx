...
const Product = () => {
  ...
  const products: ProductItem[] = useMemo(
    () =>
      (data?.data || []).filter(
        (product: ProductItem) => product.adminIsApproved === 'approve',
      ),
    [data],
  );
  ...
  const totalProducts =
    data?.meta?.total || data?.meta?.totalProducts || products.length;
  ...
  const displayCategories = useMemo(
    () =>
      categoryOptions.filter((category) =>
        VISIBLE_CATEGORY_LABELS.has(category.label),
      ),
    [],
  );
  ...
  const matchingProducts = products.filter((product) =>
    normalize(
      [product.productTitle, product.brandName, product.category].join(' '),
    ).includes(searchQuery),
  );
  const matchingCategories = displayCategories.filter((category) =>
    normalize(category.label).includes(searchQuery),
  );
  ...
  const persistSearch = (value: string) => {
    ...
    const next = [
      term,
      ...recentSearches.filter((item) => normalize(item) !== normalize(term)),
    ].slice(0, 5);
    setRecentSearches(next);
    window.localStorage.setItem(
      'quickbuzz-recent-searches',
      JSON.stringify(next),
    );
  };
  ...
  const selectCategory = (label: string) => {
    ...
    navigate(
      `/product${nextCategory === 'all' ? '' : `?category=${encodeURIComponent(nextCategory)}`}`,
    );
  };
  ...
  const activeChips: ActiveFilterChip[] = [
    ...filters.brands.map((value) => ({
      key: `brand-${value}`,
      label: value,
      remove: () => toggleListFilter('brands', value),
    })),
    ...filters.categories.map((value) => ({
      key: `category-${value}`,
      label: value,
      remove: () => toggleListFilter('categories', value),
    })),
    filters.rating
      ? {
          key: 'rating',
          label: `${filters.rating} stars & up`,
          remove: () => setFilters((current) => ({ ...current, rating: 0 })),
        }
      : null,
    filters.discount
      ? {
          key: 'discount',
          label: `${filters.discount}% off & up`,
          remove: () => setFilters((current) => ({ ...current, discount: 0 })),
        }
      : null,
    filters.inStock
      ? {
          ...
          label: 'In Stock',
          remove: () =>
            setFilters((current) => ({ ...current, inStock: false })),
        }
      : null,
    filters.freeDelivery
      ? {
          ...
          label: 'Free Delivery',
          remove: () =>
            setFilters((current) => ({ ...current, freeDelivery: false })),
        }
      : null,
